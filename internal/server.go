...
func (v1 *V1) createV1Endpoints(ctx context.Context, cancel context.CancelFunc) error {
	...
	v1.router.HandleFunc("/status", v1.statusHandler.GetStatus).Methods(http.MethodGet, http.MethodOptions)

	// Protected subrouter for all document endpoints.
	protected := v1.router.PathPrefix("").Subrouter()

	oicdEnabled, err := Cfg.OICDEnabled()
	if err != nil {
		slog.ErrorContext(ctx, "failed to validate OICD config", slog.String("error", err.Error()))
		return err
	}
	...
	protected.HandleFunc("/documents", v1.documentsHandler.GetAllDocuments).Methods(http.MethodGet, http.MethodOptions)
	protected.HandleFunc("/documents/{id}/form", v1.documentsHandler.GetDocumentForm).Methods(http.MethodGet, http.MethodOptions)
	protected.HandleFunc("/documents/{id}/generate", v1.documentsHandler.GenerateDocument).Methods(http.MethodPost, http.MethodOptions)
